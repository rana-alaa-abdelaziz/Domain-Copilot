...
## 2026-09-18
- Switched the active local development provider from Gemini to Ollama after
  Gemini reached its free-tier request limits. The local Ollama service has
  `llama3` for generation and `nomic-embed-text` for 768-dimensional embeddings,
  so the workflow can generate questions without external quota dependency.
- Restored the Claim action for review tasks assigned to another reviewer.
  The UI had hidden it whenever any assignee existed, even though the lead
  reviewer assignment endpoint supports taking ownership of the task.
- Diagnosed an empty assessment section as Gemini free-tier quota exhaustion
  (`429`), not a rendering or question-parser failure. Exposed the workflow
  error in the pending-review response and rendered an explicit message when
  the assessment report contains zero items.
- Updated the Gemini chat model from retired `gemini-1.5-flash` to the
  available `gemini-2.5-flash` after workflow generation returned a 404 from
  `generateContent`; a direct smoke test then showed this API key is not
  eligible for 2.5, so the model was corrected to Google's recommended
  `gemini-3.6-flash`.
- Corrected the Gemini embedding model after container startup failed because
  `models/text-embedding-004` was no longer available. Switched the adapter to
  `models/gemini-embedding-001` and explicitly requested 768 output dimensions
  to preserve the existing pgvector schema and retrieval contract.
- Replaced the local NVIDIA provider selection with a Gemini provider option at the user's request. Added `GeminiAdapter` behind the existing `LlmProvider` port, preserved the 768-dimensional embedding contract, and updated configuration, Compose wiring, and dependency declarations. A real `GEMINI_API_KEY` must be obtained from Google AI Studio and supplied through `.env`; no key was generated by the assistant.
